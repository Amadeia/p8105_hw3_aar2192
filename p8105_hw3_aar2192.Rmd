...
```{r filter_states_2010, cache = TRUE}
brfss_smart2010_clean %>% 
  filter(year == 2002) %>%
  group_by(state, year) %>%
...
```{r load_instacart_data, cache = TRUE}
data("instacart")
View(instacart)
...
```{r plot_products_ordered, cache = TRUE}
data_for_insta_plot =
  instacart %>% 
